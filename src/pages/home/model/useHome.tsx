import { useGameStateStore, selectors, GameState } from 'entities/GameState'
...
export const useHome = (): {startGameHandler: () => void, gameState: GameState} => {
  const gameState = useGameStateStore(selectors.gameState)
  const startGame = useGameStateStore(selectors.startGame)
  const score = useScoreStore(scoreSelectors.score)
  const time = useTimerStore(timerSelectors.time)

  useEffect(()=>{
    if (score){
      notify(<div style={{display: "flex", flexDirection:"column" ,alignItems:"flex-end"}}>
